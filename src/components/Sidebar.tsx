  ...
  return (
    <aside className="sticky top-0 flex h-screen w-56 shrink-0 flex-col border-r border-line bg-surface/70 px-3 py-5 backdrop-blur overflow-y-auto">
      <Link href="/" className="mb-6 flex items-center gap-3 px-2">
        <span className="grid h-9 w-9 place-items-center rounded-xl bg-nebula font-display text-sm font-bold text-white shadow-card">
          M2
        </span>
        <span className="leading-tight">
          <span className="block font-display text-[15px] font-semibold text-ink">Mind2Matter</span>
          <span className="block font-mono text-[10px] uppercase tracking-[0.16em] text-muted">Hale &amp; Vance Law</span>
        </span>
      </Link>

      <nav className="flex flex-col gap-0.5">
        {NAV.map((n) => (
          <Link key={n.href} href={n.href} className="nav-link" data-active={isActive(n.href)}>
            {n.icon}
            {n.label}
          </Link>
        ))}
      ...
      <div className="mt-auto rounded-xl border border-line bg-nebula-soft/50 px-3 py-3">
